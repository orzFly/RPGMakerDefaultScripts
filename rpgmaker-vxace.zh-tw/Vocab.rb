#encoding:utf-8
#
# 定義了用語和訊息。將部分資料定義為常量。用語部分來自於 $data_system 。
#
...
module Vocab
  ...
  # 關鍵一擊
  CriticalToEnemy = "關鍵一擊！"
  CriticalToActor = "痛恨一擊！"

  # 角色物件的行動結果
  ActorDamage     = "%s受到了%s點的傷害！"
  ActorRecovery   = "%s的%s還原了%s點！"
  ActorGain       = "%s的%s還原了%s點！"
  ...
  EnemyRecovery   = "%s的%s還原了%s點！"
  EnemyGain       = "%s的%s還原了%s點！"
  ...
  # 出錯時的訊息
  PlayerPosError  = "沒有設定玩家的初始位置。"
  EventOverflow   = "呼叫的公共事件超過過上限。"
  ...
  def self.fight;       command(0);   end   # 戰鬥
  ...
  def self.shutdown;    command(20);  end   # 離開游戲
