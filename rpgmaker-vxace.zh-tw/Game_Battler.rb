...
class Game_Battler < Game_BattlerBase
  ...
  #
  #
  EFFECT_RECOVER_HP     = 11              # 還原 HP
  EFFECT_RECOVER_MP     = 12              # 還原 MP
  ...
  def item_hit(user, item)
    rate = item.success_rate * 0.01         # 取得成功幾率
    rate *= user.hit if item.physical?      # 物理攻擊：計算成功幾率的乘積
    return rate                             # 返回計算後的成功幾率
  end
  #
  # 計算技能／物品的閃避幾率
  #
