...
#
# 事件指令的解釋器。
# 本類在 Game_Map、Game_Troop、Game_Event 類的內定使用。
#
...
class Game_Interpreter
  ...
  # 讀取案例
  #
  # obj : marshal_dump 中儲存的案例（數組）
  # 還原多個資料（@depth、@map_id 等）的狀態，必要時重新建立纖程。
  #
  ...
  # 迭代角色（ID）
  #
  # param : 大於 1 則返回 ID 特殊的角色、0 則迭代全體角色
  # 注意：此方法和 iterate_actor_index(param) 的參數設定有所不同
  #
  ...
  # 取得事件
  #
  # param : -1 則玩家、0 則本事件、其他 則是特殊的事件ID 
  #
  ...
  def command_101
    ...
    while next_event_code == 401       # 文字資料
      @index += 1
