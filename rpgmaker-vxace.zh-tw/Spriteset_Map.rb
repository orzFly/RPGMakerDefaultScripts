#encoding:utf-8
#
# 處理地圖畫面精靈和圖塊的類。本類在 Scene_Map 類的內定使用。
#
...
class Spriteset_Map
  ...
  def update_parallax
    if @parallax_name != $game_map.parallax_name
      ...
      @parallax.bitmap.dispose if @parallax.bitmap
      @parallax.bitmap = @parallax_name.empty? ? nil : Cache.parallax(@parallax_name)
      Graphics.frame_reset
    end
    @parallax.ox = $game_map.parallax_ox(@parallax.bitmap) if @parallax.bitmap
    @parallax.oy = $game_map.parallax_oy(@parallax.bitmap) if @parallax.bitmap
  end
  #
  # 更新人物精靈
  #
